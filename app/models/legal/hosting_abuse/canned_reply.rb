class Legal::HostingAbuse::CannedReply
  ...
  def resource_sites
    return [] unless @abuse.resource
    @abuse.resource.resource_consuming_websites
  end
  ...
  def get_template_name
    ...
    if @abuse.type.name == 'Resource Abuse'
      ...
      return 'resource_abuse_disc.txt.erb'       if @abuse.resource.type.name == 'Disk Space' && @abuse.resource.disk_abuse_type_id == 1
      return 'resource_abuse_db_overuse.txt.erb' if @abuse.resource.type.name == 'Disk Space' && @abuse.resource.disk_abuse_type_id == 2
      return 'resource_abuse_db_growth.txt.erb'  if @abuse.resource.type.name == 'Disk Space' && @abuse.resource.disk_abuse_type_id == 3
    end
