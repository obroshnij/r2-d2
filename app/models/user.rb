 class User < ActiveRecord::Base
  ...
  def self.from_ldap_entry entry
    user = create_with(email: entry.mail.first).find_or_create_by(uid: entry.samaccountname.first)
  ...
  def self.get_group_ids entry
    names = (entry.try(:memberof) || []).map do |group_dn|
      group_dn.include?('OU=Groups') ? group_dn.split(',').first.split('=').last : nil
    end
